...
const CompanyOverview = ({ className, title, image, description }: { className: string, title: string, image: string, description: string[] }) => {
    return (
        <div className="min-h-[30rem] px-4 md:px-10 py-5 md:py-10">
            <h2 className="font-sans text-[20px] md:text-3xl text-center font-bold text-cyan-600 dark:text-cyan-400 mb-2 mt-2 block md:hidden">
                {title}
            </h2>
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-4`}>
                <div className="flex flex-col justify-center h-full w-full">
                    <h2 className="font-sans text-[20px] text-center md:text-3xl font-bold text-cyan-600 dark:text-cyan-400 mb-2 mt-2 hidden md:block">
                        {title}
                    </h2>
