...
function Navbar({ scrollY }: NavbarProps) {
    ...
    return (
        <nav className={`fixed top-0 left-0 w-full z-50 transition-colors duration-50 ${navbarClass}`}>
            <div className="px-6 py-5 mx-auto md:flex md:justify-between md:items-center">
                <div className="flex items-center justify-between">
                    <a href="#">
                        <span className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-cyan-200 to-cyan-600">
                            TechNova
                        </span>
                    </a>
                ...
                <div
                    className={`absolute inset-x-0 z-20 w-full px-6 py-4 transition-all duration-300 ease-in-out md:mt-0 md:p-0 md:top-0 md:relative md:bg-transparent md:w-auto md:opacity-100 md:translate-x-0 md:flex md:items-center font-medium ${isOpen
                        ? "translate-x-0 opacity-100 bg-neutral-900"
                    ...
                    <div className="flex flex-col md:flex-row md:mx-6">
                        {menuItems.map((item, index) => (
                            <a
                                key={index}
                                className="my-2 text-gray-300 transition-colors duration-300 transform  hover:text-cyan-400 md:mx-4 md:my-0"
                                href={item.href}>
                                {item.name}
                            </a>
                        ))}
